...
def run():
  ...
  # --- TAB 1: MANUAL ENTRY FORM (Linked via Dropdowns) ---
  with tab_entry:
    st.markdown(
        "### 📝 Add New Store Inward Entry (Linked with Master Lists)"
    )

    # Extract dropdown options from masters
    supplier_options = [s["Supplier Name"] for s in st.session_state.supplier_master]
    material_options = [m["Material Description"] for m in st.session_state.material_master]
  ...
  with tab_po:
    ...
    with st.form("po_entry_form", clear_on_submit=True):
      ...
      with c2:
        po_supplier = st.selectbox("Supplier Name", supplier_options if supplier_options else [""])
        po_material = st.selectbox("Material Description", material_options if material_options else [""])
      with c3:
  ...
  with tab_supplier_master:
    ...
    st.info("Manage your approved supplier profiles here. These automatically link to entry forms.")
    ...
    if st.session_state.supplier_master:
      ...
      edited_sup_df = st.data_editor(
          sup_df, hide_index=True, use_container_width=True, key="supplier_master_editor"
      )
  ...
  with tab_material_master:
    ...
    st.info("Manage authorized materials and codes here. These automatically populate dropdown options.")
    ...
    with st.form("material_add_form", clear_on_submit=True):
      ...
      with mc2:
        m_uom = st.selectbox("Standard UOM", ["Bags", "Cu.M", "MT", "Nos", "Kgs", "Litres", "Bundles"])
    ...
    if st.session_state.material_master:
      ...
      edited_mat_df = st.data_editor(
          mat_df, hide_index=True, use_container_width=True, key="material_master_editor"
      )
